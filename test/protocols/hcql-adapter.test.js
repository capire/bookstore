...
describe ('DELETE', () => {
  it ('deletes single entities with affected rows as result', async () => {
    ...
    expect(res.data.affectedRows ?? res.data.data ?? res.data).to.equal(1) // 1 affected row
  })
  ...
  it ('deletes multiple entities with affected rows as result', async () => {
    ...
    expect(res.status).to.equal(200)
    expect(res.data.affectedRows ?? res.data.data ?? res.data).to.equal(2) // 2 affected rows
  })
})
